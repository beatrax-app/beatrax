...
| Apple Distribution certificate | The iOS `.ipa`, signed by Bifrost — and the Mac App Store `.app`, signed in CI. One certificate, two lanes: an expiry stops both | Bifrost → Credentials → iOS, and `NATIVEPHP_MAS_IDENTITY` | 2027-07-03 | `security find-certificate -c "Apple Distribution: Wessel Verheij" -p \| openssl x509 -noout -enddate`, or `openssl pkcs12 -in <p12> -nokeys \| openssl x509 -noout -enddate` |
| Mac App Store provisioning profile | Binding the desktop bundle id to the Apple Distribution certificate for the store lane | `MAS_PROVISIONING_PROFILE_BASE64` | Unread | `security cms -D -i embedded.provisionprofile \| plutil -extract ExpirationDate raw -`. **Not yet created** — the store lane refuses to build without it, which is the correct failure |
