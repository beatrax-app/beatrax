...
| [An ordering that picks, and one that walks](an-ordering-that-picks.md) | Why a tie broken on `id` shows two devices different rows, the pick-versus-walk line the guard draws, and the reads that still fall the wrong side of it |
