...
`Ledger\Public\Support\NewestTransactionFirst::ACROSS_ACCOUNTS`, the same
content key with the account named by its IBAN: the four columns this
first sorted on are unique only *within* one account, and a merchant
charged from two of them still tied — see
