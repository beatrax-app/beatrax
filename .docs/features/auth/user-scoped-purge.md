...
A written list goes stale the first time a module adds a table. Of the 89 tables
carrying a `user_id`, 67 hold a foreign key to `users`, and not one of them
cascades — all 67 are `ON DELETE NO ACTION`, with foreign keys enforced. The
constraint therefore deletes nothing on the sweep's behalf. It refuses the
`users` delete while a single child row is still there, which is why
`sweepUnownedChildren` runs before the sweep rather than after it.

That makes a stale list loud for those 67 and silent for the rest. The other 22
carry a `user_id` and no key to `users` at all, so a list that forgot one of
them — or a table added beside them later — would leave its rows behind and
still finish green:

```
anomaly_backfill_state     deferred_op_captures       device_introductions
device_registry            hlc_clock_state            ledger_backfill_state
mobile_import_intent       mobile_notification_grant  mobile_sync_progress
notification_preferences   op_log_entries             op_log_quarantine
op_log_row_aliases         pairing_tokens             search_index_repairs
sessions                   sync_backfill_state        sync_encryption_state
sync_peer_catch_up_state   sync_sessions              sync_withheld_history
...
| `rule_actions`, `rule_conditions` | `rule_id` only, and that key is `NO ACTION` too — swept as orphans first, or deleting the rule is refused |
