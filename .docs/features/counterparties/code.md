...
- `Ledger\Public\Support\NewestTransactionFirst::ACROSS_ACCOUNTS` — the
  ...
  which owns `transactions` — once `Search`, `Import` and `Recurring`
  needed it too. `transactions.id` is a per-device autoincrement, so the
  charge it called newest was a different charge on the peer; see
