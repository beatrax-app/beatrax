...
final readonly class RateProviderRegistry
{
    ...
    {
        $raw = $this->cache->get(sprintf('fx.circuit.%s.failures', $providerKey), 0);
    ...
    {
        $cacheKey = sprintf('fx.circuit.%s.failures', $providerKey);
    ...
    {
        $this->cache->forget(sprintf('fx.circuit.%s.failures', $providerKey));
    }
}
