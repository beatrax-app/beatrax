...
<div class="max-w-5xl mx-auto px-6 py-8 space-y-8" data-testid="aliases-settings-page">
    ...
    <header class="space-y-1">
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">Aliases</h1>
        <p class="max-w-2xl text-sm text-slate-500 dark:text-slate-400">
            Friendly names you've taught beatrax for the cryptic descriptions on your statements. Edit a row's generalized pattern to widen or narrow which other transactions inherit the same friendly name.
        </p>
    </header>
    ...
    @if ($flashMessage !== '')
        ...
        >
            {{ $flashMessage }}
            <button type="button" wire:click="clearFlash" class="ml-3 text-xs underline">dismiss</button>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            @if (count($aliases) === 0)
                <div class="rounded-lg border border-slate-200 bg-slate-50 p-6 text-center dark:bg-slate-900 dark:border-slate-700">
                    <h2 class="text-lg font-semibold tracking-tight text-slate-900 dark:text-slate-100">No aliases yet</h2>
                    <p class="mx-auto mt-2 max-w-md text-sm text-slate-500 dark:text-slate-400">
                        Aliases appear here after you click the italic raw description on an import preview row and give it a friendly name.
                    </p>
                </div>
            @else
                <table class="aliases-table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Generalized pattern</th>
                            ...
                            <tr wire:key="alias-row-{{ $alias->id }}">
                                <td>
                                    <input
                                        ...
                                        value="{{ $alias->id }}"
                                        aria-label="Select alias {{ $alias->friendly_name }}"
                                    />
                                </td>
                                ...
                                <td>
                                    @if ($editingId === (int) $alias->id)
                                        <input
                                            ...
                                            aria-label="Generalized pattern"
                                ...
                                <td class="text-right">
                                    ...
                                    @else
                                        ...
                                        <button
                                            ...
                                            wire:confirm="Delete this alias? Future imports of '{{ $alias->pattern }}' will go back to the raw description."
            ...
            <div class="border-t border-slate-200 dark:border-slate-700 pt-6 mt-8 space-y-4">
                <h2 class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Backup &amp; transfer</h2>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    >Export aliases as YAML</button>
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                        Downloads <code class="font-mono">aliases.yaml</code> in the community-corpus format.
                    </p>
                </div>

                <div class="space-y-2">
                    <label for="import-aliases-file" class="block text-sm text-slate-900 dark:text-slate-100">Import from YAML</label>
                    <input
                        type="file"
                    ...
                    @if ($importDiff !== [])
                        <div class="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm dark:bg-slate-900 dark:border-slate-700 space-y-3">
                            <p class="text-slate-900 dark:text-slate-100">
                                <strong>{{ count($importDiff['new'] ?? []) }}</strong> new,
                                <strong>{{ count($importDiff['unchanged'] ?? []) }}</strong> unchanged,
                                <strong>{{ count($importDiff['conflicts'] ?? []) }}</strong> conflicts.
                            </p>
        ...
        <aside class="space-y-3" aria-label="Preview against transactions">
            <div class="alias-preview-pane" data-testid="alias-preview-pane">
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">Test against my transactions</h2>
                @if ($editingId === 0)
                    <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        Edit a row's generalized pattern to see which transactions it would match.
                    </p>
                @elseif ($previewResult === [])
                    <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Typing…</p>
                ...
                @else
                    <p class="mt-2 alias-preview-count">
                        Matches <strong>{{ $previewResult['total'] ?? 0 }}</strong> transactions in your recent history.
                    </p>
    ...
    @if ($showMergeModal)
        ...
        >
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-slate-900 space-y-4">
                <h2 id="merge-aliases-title" class="text-lg font-semibold text-slate-900 dark:text-slate-100">Merge {{ count($selectedIds) }} aliases</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">
                    The remaining row keeps its raw description; the absorbed rows are preserved in <code class="font-mono text-xs">merged_from</code>.
                </p>
                <div class="space-y-2">
                    <label for="merge-friendly-name" class="block text-sm text-slate-900 dark:text-slate-100">Friendly name</label>
                    <input
                        type="text"
                ...
                <div class="space-y-2">
                    <label for="merge-generalized-pattern" class="block text-sm text-slate-900 dark:text-slate-100">Generalized pattern</label>
                    <input
                        type="text"
                    ...
                    @if ($mergeGeneralizedPattern === '')
                        <p class="text-xs text-amber-600 dark:text-amber-400">
                            No shared 4-character prefix was found across the selected aliases — type a pattern manually before confirming.
                        </p>
                    @endif
                </div>
