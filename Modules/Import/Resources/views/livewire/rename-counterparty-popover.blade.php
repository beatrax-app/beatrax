...
<div>
    <flux:modal name="rename-counterparty" dismissible>
        <div class="space-y-6">
            <flux:heading size="lg">Rename this counterparty</flux:heading>

            <form wire:submit="save" class="space-y-5">
                <div class="space-y-1">
                    <label for="rename-popover-raw" class="block text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Mystery code (from your statement)</label>
                    <input
                        type="text"
                ...
                <div class="space-y-1">
                    <label for="rename-popover-friendly" class="block text-sm text-slate-900 dark:text-slate-100">Human-friendly name</label>
                    <input
                        type="text"
                        id="rename-popover-friendly"
                        wire:model.live="friendly"
                        placeholder="Shell — Pieter Nieuwlandstraat"
                        autofocus
                ...
                <div class="space-y-2">
                    <label class="inline-flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                        ...
                        />
                        <span>
                            <span class="block">Remember "{{ $raw }}" as this name</span>
                            <span class="block text-xs text-slate-500 dark:text-slate-400">Future imports get the same name automatically.</span>
                        </span>
                    </label>

                    @if ($remember && $generalized !== '')
                        <p class="popover-preview-line">
                            This will also rename future merchants matching <code>{{ $generalized }}*</code>.
                        </p>
                    @endif
                </div>

                <div class="space-y-1">
                    <label for="rename-popover-generalized" class="block text-xs text-slate-500 dark:text-slate-400">Generalized pattern (optional override)</label>
                    <input
                        type="text"
