...
// never traverses parents at render time. percentageOfTotal is the fraction,
// which the callers sum; wholeMinor is carried beside it so the percentage a
// reader is shown can be cut from the two integers instead.
final class TopCategoryRow extends Data
{
    public function __construct(
        ...
        public readonly int $wholeMinor,
    ) {}

    // Clamped by percent() rather than at the call site, because a bar drawn at
    // one number and announced at another is the failure this row was carrying:
    ...
    {
        return OutwardSpend::percent($this->spend->toMinor(), $this->wholeMinor);
    }
