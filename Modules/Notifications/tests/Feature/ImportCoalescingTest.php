...
function icNotificationCount(int $userId, NotificationTrigger $triggerType): int
{
...
function icNotificationBody(int $userId, NotificationTrigger $triggerType): ?string
{
    /** @var DatabaseManager $db */
    $db = app(DatabaseManager::class);
...
it('produces EXACTLY 1 import_finished row for a 500-row batch, not 500', function (): void {
    ...
    expect(icNotificationCount($user->id, NotificationTrigger::ImportFinished))->toBe(1);

    $body = icNotificationBody($user->id, NotificationTrigger::ImportFinished);
...
it('produces exactly 1 receipts_found row for a 500-row all-eml batch', function (): void {
    ...
    expect(icNotificationCount($user->id, NotificationTrigger::ReceiptsFound))->toBe(1);
    expect(icNotificationCount($user->id, NotificationTrigger::ImportFinished))->toBe(0);
});
...
it('resolves a mixed csv + eml batch to import_finished, not receipts_found', function (): void {
    ...
    expect(icNotificationCount($user->id, NotificationTrigger::ImportFinished))->toBe(1);
    expect(icNotificationCount($user->id, NotificationTrigger::ReceiptsFound))->toBe(0);
});
...
it('produces two rows for two same-second imports with DIFFERENT counts (occurrence is per-batch, not per-day)', function (): void {
    ...
    expect(icNotificationCount($user->id, NotificationTrigger::ImportFinished))->toBe(2);
...
it('never leaks user A\'s import into user B\'s inbox (cross-user)', function (): void {
    ...
    expect(icNotificationCount($userA->id, NotificationTrigger::ImportFinished))->toBe(1);
    expect(icNotificationCount($userB->id, NotificationTrigger::ImportFinished))->toBe(0);
});
