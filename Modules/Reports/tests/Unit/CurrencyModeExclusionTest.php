...
// CurrencyModeApplier over transaction-level settled_currency rows — a
// different path from FxExclusionTest, which covers account-level balances.
...
it('currencyMode=original with an account filter reports the correct currency/total, never $0.00 next to non-empty rows', function (): void {
    ...
    // Inserted first so an unfiltered `DISTINCT settled_currency` scan would
    // surface USD ahead of EUR and pick the wrong primary currency.
