...
/*
 * Smoke-test the synthesised cross-source matching fixture trio. The
 * three fixtures live under
 *   Modules/Chains/tests/fixtures/scenario-1/
 * and are generated by `scripts/synthesise_phase5_scenario.php` from a
 * locked seed. The smoke test asserts each file is parseable by its
 * production adapter so Wave 1 / Wave 2 / Wave 3 resolvers can rely on
 * them without rediscovering a malformed fixture late.
 */
...
it('parses the synthesised ICS PDF via the production IcsPdfAdapter and yields 23 transactions', function (): void {
    ...
    $eurSettledSum = 0;
    foreach ($dtos as $dto) {
        // Native amount lives in $dto->amountMinor; settled-EUR amount in
        // $dto->settledAmountMinor for FX rows, or $dto->amountMinor when
        // settledAmountMinor is null (EUR-native row).
