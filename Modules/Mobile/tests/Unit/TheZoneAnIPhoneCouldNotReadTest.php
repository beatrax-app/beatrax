...
// InstallsIos copies resources/xcode/ onto nativephp/ios/, so these two files
// ARE the shell the build compiles, before the patch above rewrites them. The
// generated tree this arm used to wait on is a byte-copy of them, and the
// bytes ship in the package every job with the mobile root installed has.
/** @return array<string, string> each shipped source, keyed by the anchor it must carry exactly once */
function iosTimezoneShippedShell(): array
{
    $sources = [
        IOS_TIMEZONE_ANCHOR => 'vendor/nativephp/mobile/resources/xcode/NativePHP/NativePHPApp.swift',
        IOS_RUNTIME_ANCHOR => 'vendor/nativephp/mobile/resources/xcode/NativePHP/Bridge/PersistentPHPRuntime.swift',
    ];

    $found = [];

    foreach ($sources as $anchor => $relative) {
        foreach ([base_path($relative), base_path('mobile-app/'.$relative)] as $candidate) {
            if (is_file($candidate)) {
                $found[$anchor] = (string) file_get_contents($candidate);
...
it('anchors on text the shipped app really carries', function (): void {
    $shipped = iosTimezoneShippedShell();

    if ($shipped === []) {
        test()->markTestSkipped('nativephp/mobile is not installed under either Composer root, so there is no shipped shell to anchor against.');
    }

    // Both runtimes ship together, so one found and the other missing is the
    // package having moved a seam — the drift this arm exists to report, and
    // a loop over one file would have passed through it.
    expect(array_keys($shipped))->toBe([IOS_TIMEZONE_ANCHOR, IOS_RUNTIME_ANCHOR]);

    foreach ($shipped as $anchor => $source) {
        expect(substr_count($source, $anchor))->toBe(1);
    }
});
