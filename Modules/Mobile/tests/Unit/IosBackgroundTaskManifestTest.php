...
it('has the mobile pull in the iOS background-task manifest before any provider boots', function (): void {
    ...
    expect(str_contains($result['output'], 'sync:mobile-pull'))->toBeTrue(
        'Nothing registered sync:mobile-pull by the time BackgroundTasksServiceProvider::boot() '
        ."reads the schedule, so iOS scheduled no background sync at all. Saw: {$result['output']}",
    );
});

// Two filters drop a schedule without failing anything. `Schedule::call()` has
// no name to dispatch, so BGTaskScheduler could not run a closure even in
// principle — skipping those is correct and there are nineteen of them. A cron
// the generator has no interval for is a different thing: a schedule somebody
// wrote, which iOS then never runs. Neither is visible from the phone, so the
// named commands that do NOT survive the trip are pinned here.
const IOS_MANIFEST_CANNOT_CARRY = [
    // `0 3 * * *` is absent from SchedulerManifestGenerator's cron map, and
    // BGTaskScheduler schedules an earliest-begin date rather than a
    // wall-clock hour, so 03:00 was never a promise it could keep.
    'db:backup --force',
];
