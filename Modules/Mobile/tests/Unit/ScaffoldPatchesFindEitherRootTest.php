...
it('still finds this repository own scaffold', function (): void {
    ...
    // Asked per platform rather than of the directory above them, which is the
    // mistake scaffoldScriptsDirectory() above exists to record. `native:install
    // ios` writes nativephp/ios and no nativephp/android, so a working tree with
    // the parent and no Android manifest is an ordinary one, not a broken probe.
    $platforms = [
        'android/app/src/main' => 'AndroidManifest.xml',
        'ios/NativePHP.xcodeproj' => 'project.pbxproj',
    ];

    $installed = [];

    foreach ($platforms as $directory => $inside) {
        $resolved = beatraxScaffoldPath($directory);
        ...
        expect(is_file($resolved.DIRECTORY_SEPARATOR.$inside))
            ->toBeTrue($directory.' resolved to something that is not the scaffold: no '.$inside.' under '.$resolved);
    }

    // mobile-app/nativephp is generated by native:install and gitignored, so a
    // clean checkout has neither — which is the case the skip exists for, and a
    // null there is the right answer rather than a failure.
    if ($installed === []) {
        expect(array_map(beatraxScaffoldPath(...), array_keys($platforms)))->toBe([null, null]);
    }
});
