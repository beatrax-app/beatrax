...
final class MobilePairingScan extends Component
{
    ...
    private function fanOutToConfirmedPeers(
        ...
        $recipients = $db->connection()->table('device_registry')
            ...
            // verifies history and collects nothing. Named with its table so an
            // absent column raises rather than answering silently; the gate
            // keeps this screen unrendered until every migration has run.
    ...
    private function hasConfirmedPeer(int $userId, DatabaseManager $db): bool
    {
        return $db->connection()
            ...
            ->where('is_self', 0)
            ->whereNotNull('confirmed_at')
            // Named with its table for the reason the fan-out above is.
            ->whereNull('device_registry.self_retired_at')
            ->exists();
    }
