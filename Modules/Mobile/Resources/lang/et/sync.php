<?php

declare(strict_types=1);

return [
    'page_title' => 'Andmed ja seadmed',
    'heading' => 'Andmed ja seadmed',
    'sync_status' => 'Sünkroonimise olek',
    'syncing_progress' => 'Sünkroonin… :count kirje|Sünkroonin… :count kirjet',
    'initial_sync_aria' => 'Esmase sünkroonimise edenemine',
    'no_peers' => 'Sünkroonimise alustamiseks seo teine seade.',
    'sync_now' => 'Sünkrooni kohe',
    'result' => [
        'synced' => 'Sünkroonitud sinu teise seadmega.',
        'unreachable' => 'Teine seade ei olnud kättesaadav — kontrolli, kas mõlemad on samas võrgus.',
        'not_secured' => 'Su teine seade vastas, kuid turvalist ühendust sellega ei õnnestunud avada. Ava selles seadmes Beatraxi lukk ja proovi uuesti; kui see ikka ei õnnestu, seo seadmed uuesti paari.',
        'locked' => 'Ava rakenduse lukk, et sünkroonida.',
        'not_enabled' => 'Sünkroonimine ei ole selles seadmes veel seadistatud.',
        'unreadable' => 'Selle seadme võti ei avane enam. Seo seadmed uuesti, et sünkroonimist jätkata.',
        'paused_on_cellular' => 'Peatatud — sünkroonimine on piiratud Wi-Fi-ga ja sa oled mobiilses andmesides.',
    ],
    'background_note' => 'Beatrax kuulab kogu aja, kui ta on avatud, nii et seotud seade saab sellega igal ajal sünkroonida. Sünkrooni kohe alustab andmevahetust siitpoolt.',
    'background_note_phone' => 'Sünkroonimine toimub siis, kui puudutad Sünkrooni kohe. Taustal see töötada ei saa — rakenduse lukk hoiab ainsat võtit.',
    'network' => 'Võrk',
    'pause_cellular' => 'Peata sünkroonimine mobiilsides',
    'pause_cellular_help' => 'Vaikimisi väljas — sünkroonimine töötab kõikjal. Lülita sisse, et sünkroonida ainult Wi-Fi kaudu.',
];
