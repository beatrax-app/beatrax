...
<div class="max-w-lg mx-auto px-6 py-8 space-y-6" data-testid="sync-screen">
    ...
    <h1 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Sync</h1>

    <section class="space-y-3">
        <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100">Your devices</h2>
        ...
        @if ($initialSyncInProgress)
            <div class="space-y-2" data-testid="sync-initial-progress">
                <p class="text-sm text-slate-600 dark:text-slate-400" aria-live="polite">
                    Syncing&hellip; {{ $progressApplied }} of {{ $progressExpected ?? $progressApplied }} records
                </p>
                <div
    ...
    <section class="space-y-3">
        <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100">Network</h2>

        {{-- ===== "Pause sync on cellular" toggle (D-10) ===== --}}
        <div class="flex items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
                <p class="text-sm text-slate-900 dark:text-slate-100">Pause sync on cellular</p>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Off by default — sync works everywhere. Turn on to only sync over Wi-Fi.
                </p>
            </div>
            ...
            <div class="min-w-[44px] min-h-[44px] flex items-center justify-center">
                <button
                    ...
                    aria-label="Pause sync on cellular"
                >
