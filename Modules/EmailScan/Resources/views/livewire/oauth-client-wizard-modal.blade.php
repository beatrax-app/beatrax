            ...
            @elseif ($provider === 'microsoft')
                <ol class="space-y-6">
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white">1</span>
                        <div class="space-y-2">
                            <p class="text-sm font-semibold text-slate-900">Open Azure Portal</p>
                            <p class="text-sm text-slate-500">Open the Microsoft Entra admin center in a new tab. Sign in with the Microsoft account you want to scan.</p>
                            <a
                    ...
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500">2</span>
                        <div class="space-y-2">
                            <p class="text-sm font-semibold text-slate-900">Register a new application</p>
                            <p class="text-sm text-slate-500">Open App registrations → New registration. Name it "diederik". Under "Supported account types" choose "Accounts in any organizational directory and personal Microsoft accounts" (this lets you connect personal Outlook.com and work Microsoft 365 inboxes with the same app).</p>
                        </div>
                    </li>
                    ...
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500">3</span>
                        <div class="space-y-2">
                            <p class="text-sm font-semibold text-slate-900">Add the redirect URI</p>
                            <p class="text-sm text-slate-500">In the same registration form, under "Redirect URI", choose platform "Web" and paste the URI below exactly.</p>
                            <button
                                type="button"
                                x-data
                                x-on:click="navigator.clipboard.writeText($el.querySelector('span').textContent); $el.querySelector('span').textContent = 'Copied'; setTimeout(() => $el.querySelector('span').textContent = '{{ $redirectUri }}', 2000);"
                    ...
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500">4</span>
                        <div class="space-y-2">
                            <p class="text-sm font-semibold text-slate-900">Grant Mail.Read permission</p>
                            <p class="text-sm text-slate-500">Open API permissions → Add a permission → Microsoft Graph → Delegated permissions. Select Mail.Read and offline_access. Click Add permissions. You do not need to grant admin consent for a personal account.</p>
                        </div>
                    </li>
                    ...
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500">5</span>
                        <div class="space-y-2">
                            <p class="text-sm font-semibold text-slate-900">Create a client secret</p>
                            <p class="text-sm text-slate-500">Open Certificates & secrets → New client secret. Set description "diederik" and an expiry of 24 months. Copy the secret value immediately — Azure shows it only once.</p>
                        </div>
                    </li>
                    ...
                    <li class="flex items-start gap-4">
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500">6</span>
                        <div class="space-y-3 flex-1">
                            <p class="text-sm font-semibold text-slate-900">Paste your application (client) ID and secret</p>
                            <div class="space-y-3">
                                <div>
                                    <label class="block text-xs font-medium text-slate-500 mb-1">Application (client) ID</label>
                                    <input
                                        type="text"
                                        wire:model.live="clientId"
