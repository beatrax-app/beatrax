...
function carryoverTx(int $userId, int $accountId, int $runId, int $settledMinor, ?int $categoryId, CarbonImmutable $postedAt): void
{
    ...
    Transaction::create([
        ...
        'counterparty_name' => sprintf('CarryM%s', $i),
        'counterparty_normalized' => sprintf('carrym%s', $i),
...
function unpricedSpendRow(int $userId, int $accountId, int $runId, int $categoryId, CarbonImmutable $postedAt, string $currency, string $type, int $settledMinor): void
{
    ...
    Transaction::create([
        ...
        'counterparty_name' => sprintf('Unpriced%s', $j),
        'counterparty_normalized' => sprintf('unpriced%s', $j),
        'normalization_version' => 1,
        'category_id' => $categoryId,
        'source_format' => 'camt053',
        'import_run_id' => $runId,
