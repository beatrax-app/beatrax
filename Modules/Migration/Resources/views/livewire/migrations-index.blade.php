<div class="space-y-6">
    @if ($runs->isEmpty())
        <div class="flex flex-col items-center gap-4 rounded-md border border-slate-200 bg-slate-50 px-6 py-16 text-center dark:bg-slate-900 dark:border-slate-700">
            <h1 class="text-2xl font-semibold text-slate-900 tracking-tight dark:text-slate-100">No migrations yet</h1>
            <p class="max-w-md text-sm text-slate-500 dark:text-slate-400">
                Bring your category tree, budget history, and transactions over from YNAB4, new YNAB, or Actual Budget. Nothing is written to your ledger until you review and confirm.
            </p>
            <a
    ...
    @else
        <header class="flex items-baseline justify-between gap-4">
            <h1 class="text-2xl font-semibold text-slate-900 tracking-tight dark:text-slate-100">Migrations</h1>
            <a
                href="{{ route('migrations.new') }}"
                class="inline-flex items-center rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2 dark:hover:bg-emerald-400 dark:bg-emerald-500"
            >
                ...
                <li class="flex items-center justify-between gap-4 px-4 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        @elseif ($run->status === \Modules\Migration\Public\Enums\MigrationRunStatus::NeedsAttention->value)
                            <span class="inline-flex items-center rounded-md bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-600/20 dark:bg-amber-950">Needs attention</span>
                        @else
                            <span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">Parsed</span>
                        @endif
                    </div>
