...
| `MenuBar` | ABSENT | — | No `fake()` method and no `*Fake` class. System-tray (`MenuBar`) wiring is verified two ways: the module builder output is asserted in `TrayMenuBuilderTest`, and the boot-time wire-level payload to NativePHP's `menu-bar/create` endpoint is asserted via `Http::fake()` request recording in `NativeAppServiceProviderTest` (catches regressions like dropping `onlyShowContextMenu(true)`). Visual rendering of the Tray itself is still manual-UAT. |
...
be fully automated via `Window::fake()`; the app-menu and system-tray
tests assert the builder output (the module's own classes) automatically
and mark the live-facade leg `->todo()`.
