 ...
 * to a focused BrowserWindow and breaks "Open diederik" once the user
...
it('injects the persistent-tray marker comment exactly once', function (): void {
    ...
    expect(substr_count($patched, '// ── diederik persistent menu-bar tray ──'))->toBe(1);
});
...
it('builds the verbatim D-09 three-row context menu — Open diederik / Scan email now / Quit', function (): void {
    $upstream = "import { app } from 'electron';\nNativePHP.bootstrap(app);\n";

    [$patched] = injectPersistentTray($upstream);

    expect($patched)->toContain("label: 'Open diederik'");
...
it('is idempotent — a source containing the marker is returned unchanged', function (): void {
    $alreadyPatched = <<<'JS'
        ...
        // ── diederik persistent menu-bar tray ──
        let tray = null;
        JS;
...
it('splices the injection AFTER the bootstrap statement, not before', function (): void {
    ...
    $injectionPos = strpos($patched, '// ── diederik persistent menu-bar tray ──');
