...
// before a route existed decodes to null exactly as it would on disk. The
// factory is injected rather than reached for through the Http facade, which
// no class under Modules/ may name, and GuzzleHttp is contained to the two
// modules that speak to a bank -- so this is the one way left to build one.
final class StubElectronApi extends Client
{
    ...
    public function get(string $endpoint, array|string|null $query = null): Response
    {
        ...
        $this->http->fake(['*' => $this->http->response($this->body, $this->status)]);

        return $this->http->get(self::UNREACHABLE_HOST.$endpoint);
    }
}
