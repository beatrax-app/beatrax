...
<div>
    @if ($openCount > 0)
        <a
            ...
            aria-label="{{ Lang::get('drift-alerts::dashboard.aria', ['count' => $openCount, 'amount' => $totalFormatted]) }}"
        >
            ...
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400" style="font-variant-numeric: tabular-nums;">
                {{ Lang::get('drift-alerts::dashboard.open') }} · ↗ {{ $totalFormatted }} {{ Lang::get('drift-alerts::dashboard.annualized_impact') }}
            </p>
