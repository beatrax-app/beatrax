    ...
    @case ('backup_overdue')
        @php
            ...
            $hoursOld = isset($metadata['hours_old']) && is_numeric($metadata['hours_old'])
                ...
                : 0;
        @endphp
        The most recent verified backup is {{ $hoursOld }}h old. Run <code class="rounded bg-amber-100 px-1 text-amber-900">php artisan db:backup</code> or wait for the 03:00 scheduled run.
        @break
    @case ('wal_mode_missing')
        @php
            ...
            $currentMode = isset($metadata['current_mode']) && is_string($metadata['current_mode'])
                ...
                : 'unknown';
        @endphp
        SQLite is not in WAL mode (currently {{ $currentMode }}). Concurrent writes may stall. Run <code class="rounded bg-amber-100 px-1 text-amber-900">php artisan diederik:doctor</code> for guidance.
        @break
