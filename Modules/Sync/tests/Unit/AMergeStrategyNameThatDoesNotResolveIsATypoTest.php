...
it('keeps the five fields that are not last-writer-wins', function (): void {
    ...
    expect($registry->strategyFor('merchant_memories', 'occurrence_count'))->toBe(MergeStrategy::GCounter)
        ...
        ->and($registry->strategyFor('counterparties', 'metadata'))->toBe(MergeStrategy::JsonKeyUnion)
        ->and($registry->strategyFor('users', 'community_settings'))->toBe(MergeStrategy::JsonKeyUnion)
        ->and(declaredMergeStrategies())->toHaveCount(5);
});
