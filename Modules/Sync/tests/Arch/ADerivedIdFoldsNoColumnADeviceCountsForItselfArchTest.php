...
function derivedIdentityTuples(): array
{
    ...
    foreach (derivedIdentitySourceFiles() as $path) {
        ...
        $found = PatternScan::all("/DerivedRowId::for\(\s*'([a-z_]+)'\s*,\s*\[(.*?)\]/s", $source);
...
it('reads a non-empty set of derived identity tuples off a schema that answers', function (): void {
    expect(derivedIdentityTuples())->not->toBeEmpty(
        'No DerivedRowId::for() call site was found, so the rule below walks nothing.'
    );
