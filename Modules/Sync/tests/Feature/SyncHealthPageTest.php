...
it('renders the 7-day count correctly in the console pane header (rose when >0)', function (): void {
    ...
    expect(RenderedMarkup::of($html)->firstOrFail('[data-testid="quarantine-count"]')->text())
        ->toBe('2 skipped ops');
});
...
it('renders the calm empty state when the acting user has zero quarantine rows', function (): void {
    ...
    expect(RenderedMarkup::of($html)->firstOrFail('[data-testid="quarantine-count"]')->text() === '0 skipped ops')->toBeTrue(
