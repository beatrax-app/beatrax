...
const REGISTERED_WHOLE_STRUCTURAL = [
    ...
    'migration_staging_transactions.payee_source_external_id' => 'Holds the SOURCE PRODUCT\'s payee id, not a payee — NynabParserTest matches it against a payee id, and the name itself lives in migration_staging_payees.normalized_name. It matches the token through "payee" alone.',
...
const REGISTERED_WHOLE_OPEN = [
    'migration_staging_transactions.description' => 'The plaintext description off the export, and the pre-image of the sealed transactions.description PromoteStagingToDomain writes from it. It is not transient: DiscardMigrationRun clears staging through DependentRowCascade, ConfirmMigration does not, so a SUCCESSFUL run leaves one readable copy per migrated transaction standing beside the sealed column it became. ThreeWayMergeResolver reads staging back during a reconcile, so the retention is at least partly deliberate and the answer is to seal it, not to delete it.',
    'migration_staging_payees.normalized_name' => 'The payee name off the export. PromotionMaps reads this table to build payeeNames and PromoteStagingToDomain passes it as counterpartyName, which is SEALED on the transaction — so this is the pre-image of transactions.counterparty_name, and it outlives the run for the same reason description beside it does.',
