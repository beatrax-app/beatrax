...
final class DevicesAndSyncSettingsSection extends Component
{
    ...
    // Rung two of the transport ladder, as `host:port`. Empty means the ladder
    // has two rungs on this device, not three: discovery, then a guess at the
    // peer derived from the relay endpoint's host.
    ...
    // The one rung of the transport ladder a reader can supply. A network that
    // blocks mDNS leaves discovery with nothing to return, and without this the
    // dial falls through to an address guessed from the relay endpoint's host.
    // An empty host takes the rung back out.
    public function saveManualPeerAddress(
        ...
        $parsed = self::parseAddress($typed);
    ...
    // parse back out. Split at the LAST colon, which is the port separator on
    // every form this accepts.
    /**
     * @return array{host: string, port: int}|null
     */
    private static function parseAddress(string $typed): ?array
    {
        $at = strrpos($typed, ':');
    ...
    // One peer, matching what the dial itself assumes: PeerLanAddress reads
    // the first other device and no other, so a second one here would offer a
