...
final readonly class SyncCaptureListener
{
    ...
    public function __construct(
        private OpCaptureSinkFactory $sinks,
        private LoggerInterface $log,
        private MergeRulesRegistry $rules,
    ) {}

    // Error, with no quiet arm left. Sync being off and the app being locked
    // used to arrive here as an unbuildable writer and were logged at debug to
    // keep 120k ordinary lines out of the log; the sink factory answers both of
    // them now, so anything still reaching this is a mutation nobody will send.
    /**
     * @param  array<string, scalar|null>  $context
     */
    private function report(string $message, \Throwable $e, array $context): void
    {
        $this->log->error($message, [...$context, ...SafeExceptionContext::describe($e)]);
    }

    public function handle(TransactionMutated $event): void
    {
        try {
            // Inside the try, because a queue insert can fail too and the
            // listener runs in the request that saved the row: an escape here
            // fails the reader's edit over replication they did not ask for.
            $writer = $this->sinks->forUser($event->userId);
    ...
    // Routes TransactionSplitMutated events to the capture sink with table:
    ...
    // Routes EnvelopeAssignmentMutated events to the capture sink with table:
    ...
    // Routes EnvelopeMoveMutated events to the capture sink with table:
    ...
    // Routes GoalContributionMutated events to the capture sink with table:
    ...
    private function writeEntityEdit(EntityMutated $event, OpCaptureSink $writer): void
    {
    ...
    // Routes GoalMutated events to the capture sink with table: 'goals'.
    ...
    // Routes EnvelopeSettingMutated events to the capture sink with table:
    ...
    // Routes SavedReportMutated events to the capture sink with table:
    ...
    // Routes NotificationMutated events to the capture sink with table:
    ...
    // Routes NotificationPreferenceMutated events to the capture sink with
