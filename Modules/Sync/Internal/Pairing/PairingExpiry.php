...
// pairing_tokens.expires_at is TEXT compared lexically in SQL, so every value
// written and every value compared against has to share one zero-padded Zulu
// format — an offset form sorts by its own local hour digits (see @link).
/**
 * @link ../../../../.docs/features/sync/pairing-handshake.md#expiry-is-compared-as-text
 */
final class PairingExpiry
{
    ...
    public static function stamp(CarbonImmutable $moment): string
    {
        ...
        if (preg_match(self::ZULU_PATTERN, $stamp) !== 1) {
            throw new LogicException(
                "PairingExpiry: timestamp '{$stamp}' is not zero-padded UTC Zulu ISO8601. "
