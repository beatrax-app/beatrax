...
final class SensitiveFieldRegistry
{
    ...
    public static function knowinglyPlaintext(): array
    {
        return [
            ...
            'community_merchant_mappings.pattern' => 'The raw description a reader suggests a name for, and the corpus\'s identity column: it carries a partial UNIQUE over the global tier plus unique(user_id, pattern), and ContributionLog upserts on the latter, so ciphertext would file every correction as a second suggestion. It is also a verbatim copy of the sealed transactions.description — the triage row dispatches its decrypted description straight into the modal that writes this. Measured on an enrolled install: three rows holding bank descriptions in the clear beside a base64 column.',
            'community_merchant_mappings.generalized_pattern' => 'The normalised form of that same string, and the needle CommunityCorpusQuery loads its lookup from, behind a where(generalized_pattern, !=, \'\') predicate. It is the half that would have been left readable had only pattern been sealed, which is the shape recurring_series.display_name_override is on this list for.',
            'community_merchant_mappings.name' => 'The merchant name the corpus answers with. 7,060 of the 7,063 rows are the shipped global tier and carry user_id IS NULL, while the codec keys on a user — there is no key to seal them under, the same argument categories.slug is here for. The reader\'s own three rows hold their naming of a merchant, which is merchants.name\'s standing.',
            'community_merchant_mappings.support_email' => 'A merchant\'s published support address, one of five CONTACT_COLUMNS the corpus answers a profile with. Shipped reference data about a business rather than anything the reader wrote: 141 global rows carry one and no user row does. The global tier has no key to seal it under either.',
            ...
            'statement_summaries.iban_owner' => 'The statement\'s OWN iban, not an owner name — Camt053Adapter and Mt940StatementAccumulator both fill it from $ownIban. It is therefore the same value accounts.iban already holds in the clear two entries up, and sealing it here would leave that readable copy one table over for no gain. Same argument accounts.name is on this list for.',
            ...
            'pending_enrichment_conflicts.stored_value' => 'The decrypted transactions.counterparty_name or description a held receipt conflict is about, so the prompt shows what the ledger holds rather than base64. Argued in *The knowingly-accepted exceptions* on the linked page since before this entry existed and absent from here — the same docs-yes/registry-no half that paragraph says it closed, standing in the paragraph that says it. FingerprintStage opens the row and ApplyEnrichments writes the opened value, so the copy is made across two files. Measured on an enrolled install: one row held a description in the clear beside a base64 column.',
            'pending_enrichment_conflicts.incoming_value' => 'The receipt\'s own value for that same field, and the only copy of it: ApplyReceiptConflictResolution reads it back and seals it onto the transaction, so a sealed copy here would be opened and re-sealed on the way through a row that is deleted on resolve. Pinned plaintext by ApplyEnrichmentsEncryptionTest. For the other two conflict fields, currency and amount_minor, neither column shadows anything sensitive.',
