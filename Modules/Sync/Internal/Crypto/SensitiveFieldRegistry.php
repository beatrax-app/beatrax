...
final class SensitiveFieldRegistry
{
    ...
    public static function knowinglyPlaintext(): array
    {
        return [
            ...
            'merchant_aliases.generalized_pattern' => 'A predicate nowhere — the resolver reads every row for the user and matches in PHP — but it is the generaliser run over merchant_aliases.pattern, which carries the UNIQUE above and provably cannot be sealed. Sealing a string beside the one it was derived from is the objection that keeps accounts.name next to accounts.slug.',
            ...
            'inbox_messages.sender_name' => self::unreachableFromTheQueue('The display name off the From header, matched by nothing and indexed by nothing.'),
