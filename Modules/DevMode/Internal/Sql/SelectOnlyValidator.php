...
/**
 * Parse-time guard for the /dev/sql SELECT-only panel.
 *
 * **THIS CLASS IS THE SINGLE SEAM** for the doctrine/sql-formatter
 * library inside the codebase. The library's {@see Tokenizer} class
 * is marked `@internal` (verified at
 * github.com/doctrine/sql-formatter/blob/master/src/Tokenizer.php).
 * The mitigation strategy is two-sided:
 *
 *   1. Wrap the tokenizer's instantiation + iteration here so only
 *      ONE file in the project references the @internal class.
