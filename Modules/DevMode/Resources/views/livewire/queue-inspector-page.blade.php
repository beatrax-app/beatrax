        ...
        @else
            @php
                $tabTotal = match ($tab) {
                    ...
                    'failed' => $failedCount,
                    default => $batchesCount,
                };
            @endphp
            {{-- The tiles above count every row the queue holds and the loader
                 takes the newest hundred, so 795 failed jobs sat over a list of
                 100 with nothing between them saying so. --}}
            @if ($tabTotal > count($rows))
                <p class="px-3 pt-3 text-xs text-[var(--color-text-muted)]" data-testid="queue-truncated">
                    {{ Lang::get('core::components.showing_recent', ['shown' => Fmt::number(count($rows)), 'count' => Fmt::number($tabTotal)]) }}
                </p>
            @endif
                ...
                <thead class="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
                    <tr class="text-left text-xs uppercase text-slate-500 dark:text-slate-400">
                        ...
                        @if ($tab === 'pending')
                            ...
                            <th class="px-3 py-2">{{ Lang::get('dev::queue.col_state') }}</th>
                    ...
                    @foreach ($rows as $row)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/40" data-testid="queue-row" data-row-key="{{ $rowKey }}">
                            ...
                            @if ($tab === 'pending')
                                ...
                                @php
                                    // reserved_at means a worker holds this job right now; an
                                    // available_at in the future means it is not due yet. Neither
                                    // was read, so all three states drew the same row and the
                                ...
                                <td class="px-3 py-2 text-xs" data-testid="row-state">
                                    @if ($isReserved)
                                        <span class="text-amber-600 dark:text-amber-400">{{ Lang::get('dev::queue.state.reserved') }}</span>
                                        <span class="block text-slate-500 dark:text-slate-400">{{ \Carbon\CarbonImmutable::createFromTimestamp($reservedAt)->diffForHumans() }}</span>
                                    @elseif ($isScheduled)
                                        <span class="text-slate-700 dark:text-slate-300">{{ Lang::get('dev::queue.state.scheduled') }}</span>
                                        <span class="block text-slate-500 dark:text-slate-400">{{ \Carbon\CarbonImmutable::createFromTimestamp($availableAt)->diffForHumans() }}</span>
                                    @else
                                        <span class="text-slate-700 dark:text-slate-300">{{ Lang::get('dev::queue.state.available') }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2 text-right">
                                    @if ($isReserved)
                                        {{-- Deleting the row a worker is mid-execution on does not
                                             stop the work; it only loses the record of it. --}}
                                        <span class="text-xs text-slate-500 dark:text-slate-400" data-testid="row-delete-withheld">{{ Lang::get('dev::queue.worker_running') }}</span>
                                    @else
                                        <button
                                            type="button"
