    ...
    @if ($hasMore || $isPaged)
        <nav class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400" aria-label="{{ Lang::get('dev::audit.pagination_aria') }}">
            <button
                ...
                wire:click="newer"
                @disabled(! $isPaged)
                class="rounded border px-3 py-1 font-medium {{ $isPaged ? 'border-slate-200 bg-white hover:bg-slate-50 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800' : 'cursor-not-allowed border-slate-100 bg-slate-50 text-slate-600 dark:bg-slate-950 dark:border-slate-800 dark:text-slate-600' }}"
