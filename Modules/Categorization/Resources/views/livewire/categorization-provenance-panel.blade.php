...
<div>
    @if ($variant === 'rule')
        ...
        >
            <h3 id="provenance-heading-{{ $transactionId }}" class="text-sm font-semibold text-slate-900">
                Rule that fired
            </h3>
            ...
            @else
                <div class="mt-3 flex items-center gap-2">
                    <button
                        ...
                        class="inline-flex items-center rounded-md px-2 py-1 text-sm font-medium text-slate-900 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2"
                    >Update rule</button>
                    <button
                        type="button"
                        wire:click="confirmRemove"
                        aria-label="Remove rule (also deletes it from the Rules page)"
                        class="inline-flex items-center rounded-md px-2 py-1 text-sm font-medium text-rose-600 hover:bg-rose-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-600 focus-visible:ring-offset-2"
                    >Remove rule</button>
                </div>
            @endif
        </section>
    @elseif ($variant === 'memory')
        <section
            ...
            class="rounded-md border border-slate-200 bg-slate-50 p-4"
        >
            <h3 id="provenance-heading-{{ $transactionId }}" class="text-sm text-slate-500">
