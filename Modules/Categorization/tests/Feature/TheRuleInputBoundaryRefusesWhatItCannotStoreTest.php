...
function ruleBoundaryUser(): User
{
 ...
 */
function ruleBoundaryInput(array $conditions, array $actions, string $combinator = 'all'): RuleInput
{
...
it('stores a rule whose every field is one it recognises', function (): void {
    $ruleId = ($this->create)($this->user, ruleBoundaryInput(ruleBoundaryCondition(), $this->action));
...
it('refuses input it cannot store, naming the field it refused', function (array $conditions, array $actions, string $combinator, string $needle): void {
    expect(fn () => ($this->create)($this->user, ruleBoundaryInput($conditions, $actions, $combinator)))
        ->toThrow(InvalidArgumentException::class, $needle);
})->with([
    'combinator outside the enum' => [
        fn () => ruleBoundaryCondition(),
...
it('asks for a condition and an action in the reader own language rather than raising', function (): void {
    expect(fn () => ($this->create)($this->user, ruleBoundaryInput([], $this->action)))
        ->toThrow(ValidationException::class);

    expect(fn () => ($this->create)($this->user, ruleBoundaryInput(ruleBoundaryCondition(), [])))
        ->toThrow(ValidationException::class);
});
...
it('writes nothing at all when it refuses', function (): void {
    ...
    $refusals = [
        fn () => ($this->create)($this->user, ruleBoundaryInput(ruleBoundaryCondition(), $this->action, 'xor')),
        fn () => ($this->create)($this->user, ruleBoundaryInput([['field' => 'iban', 'op' => 'contains', 'value_type' => 'string', 'value' => 'x']], $this->action)),
        fn () => ($this->create)($this->user, ruleBoundaryInput(ruleBoundaryCondition(), [['type' => 'note', 'payload' => ['text' => '']]])),
        fn () => ($this->create)($this->user, ruleBoundaryInput(ruleBoundaryCondition(), [
...
it('takes a bound at the ceiling and refuses the one unit past it', function (): void {
    ...
    $ruleId = ($this->create)($this->user, ruleBoundaryInput(
    ...
    expect(fn () => ($this->create)($this->user, ruleBoundaryInput(
