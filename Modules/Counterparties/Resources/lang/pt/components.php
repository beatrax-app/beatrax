<?php

declare(strict_types=1);

return [
    'type_chip' => [
        'aria' => 'Tipo de contraparte: :type',
        'merchant' => 'Comerciante',
        'personal' => 'Pessoal',
        'bank' => 'Banco',
        'government' => 'Estado',
        'self' => 'Própria',
        'unknown' => 'Desconhecida',
    ],

    'filter_chips' => [
        'aria' => 'Filtrar por tipo',
        'all' => 'Todas',
        'merchant' => 'Comerciantes',
        'personal' => 'Pessoais',
        'bank' => 'Bancos',
        'government' => 'Estado',
        'self' => 'Próprias',
        'unknown' => 'Desconhecidas',
    ],

    'default_name' => [
        'bank_fee' => 'Comissão bancária',
        'account_maintenance' => 'Comissão de manutenção de conta',
        'monthly_fee' => 'Mensalidade',
        'quarterly_fee' => 'Comissão trimestral',
        'annual_fee' => 'Anuidade',
        'card_fee' => 'Comissão de cartão',
        'transaction_fee' => 'Comissão por transação',
        'transfer_fee' => 'Comissão de transferência',
        'withdrawal_fee' => 'Comissão de levantamento',
        'transaction_levy' => 'Imposto sobre transações',
        'foreign_transaction_fee' => 'Comissão de câmbio',
        'commission' => 'Comissão',
        'debit_interest' => 'Juros devedores',
        'overdraft' => 'Comissão de descoberto',
        'overdraft_interest' => 'Juros de descoberto',
        'insufficient_funds' => 'Comissão por devolução',
        'penalty_fee' => 'Penalização',
        'loan_arrangement_fee' => 'Comissão de dossiê',
    ],

    'cp_card' => [
        'aria' => 'Contraparte: :name',
        'recent_aria' => 'Atividade recente',
    ],

    'chain_flow' => [
        'aria' => 'Cadeia de financiamento: :chain',
        'join' => ' para ',
    ],

    'iban_row' => [
        'label' => 'IBAN',
        'hidden_aria' => 'IBAN oculto — clica em Mostrar IBAN para o ver',
        // i18n-review: pt · hidden_aria_touch — the same line for a touch
        // screen; check the verb governs this case.
        'hidden_aria_touch' => 'IBAN oculto — toca em Mostrar IBAN para o ver',
        'show' => 'Mostrar IBAN',
        'hide' => 'Ocultar IBAN',
    ],

    'privacy_banner' => [
        'aria' => 'Aviso de privacidade para contactos pessoais',
        'body' => '🔒 Este é um contacto pessoal. O IBAN fica oculto até o mostrares e não entra nas exportações. O nome continua a aparecer onde quer que apareçam as transações deste contacto.',
    ],

    'self_stub' => [
        'aria' => 'Não é uma contraparte real',
        'heading' => 'Isto não é realmente uma contraparte',

        'body_rest_html' => ' aparece aqui porque surge nas tuas transações como o troço de financiamento entre contas. Mas é <strong>a tua própria conta</strong>, não alguém com quem transacionas.',
        'body2' => 'Abre a vista da conta para veres o saldo, os extratos e o histórico completo de transações.',
        'open_cta' => 'Abrir a vista de conta de :name →',
        'hide_cta' => 'Ocultar desta lista',
        'recent_legs' => 'Troços recentes entre contas',
    ],
];
