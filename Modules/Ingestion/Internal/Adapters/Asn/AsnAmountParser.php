 ...
 * and an optional leading sign — e.g. "-12.34", "0.29", "1234567.89".
 *
 * The parser is integer-only by construction (Pitfall 1): the regex captures
 * the whole and fractional groups and combines them via `($whole * 100 +
 * $fractional) * $sign`. No `(float)`, no `round()`, no `intval` of a float.
 * That is the reason "0.29" returns exactly 29 instead of the silent 28 that
 * `(int)((float) '0.29' * 100)` would produce on 64-bit IEEE-754.
 */
