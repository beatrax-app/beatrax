...
@php
    ...
    $detectorLabels = ['large' => 'Large charge', 'first_time' => 'First time', 'duplicate' => 'Duplicate'];
@endphp
...
<div class="space-y-6" data-testid="anomaly-settings-section">
    <form wire:submit="save" class="space-y-6">
        {{-- Sensitivity --}}
        <div class="space-y-1">
            <label for="anomalySensitivityPercent" class="block text-sm text-slate-900 dark:text-slate-100">Alert sensitivity</label>
            <input
                type="number"
                min="1"
                max="100"
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Flag charges more than {{ $anomalySensitivityPercent }}% above your typical spend for that merchant or category.
            </p>
        </div>

        {{-- Minimum charge amount floor --}}
        <div class="space-y-1">
            <label for="anomalyMinAmountMinor" class="block text-sm text-slate-900 dark:text-slate-100">Minimum charge amount</label>
            <input
                type="number"
                min="0"
                step="1"
            ...
            />
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Ignore anomalies on charges under this amount. Stored in cents (€) — 1000 means €10.00.
            </p>
        </div>
        ...
        >Save anomaly settings</button>

        @if ($saved)
            <p wire:transition.duration.4000ms class="text-sm text-emerald-700 dark:text-emerald-400" data-testid="anomaly-saved">Saved.</p>
        @endif
    </form>
    ...
    <details class="mt-2" data-testid="suppression-rules">
        ...
        @if (count($suppressionRules) === 0)
            <p class="mt-2 text-xs text-slate-500 dark:text-slate-400" data-testid="suppression-rules-empty">
                No suppression rules yet. When you mark a charge as expected, a rule appears here.
            </p>
        @else
                ...
                @foreach ($suppressionRules as $rule)
                    <li class="flex items-center justify-between gap-3 py-2" data-testid="suppression-rule-{{ $rule->id }}">
                        <span class="min-w-0 flex-1 truncate text-sm text-slate-700 dark:text-slate-300" style="font-variant-numeric: tabular-nums;">
                            {{ $rule->displayName !== '' ? $rule->displayName : 'Unknown merchant' }}
