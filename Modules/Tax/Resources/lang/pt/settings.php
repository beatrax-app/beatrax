<?php

declare(strict_types=1);

return [
    'country_moved_desc' => 'O teu país está agora junto ao idioma, em Apresentação. Continua a determinar que categorias de dedução são oferecidas aqui.',
    'country_moved_link' => 'Alterar o país',

    'categories_label' => 'Categorias de dedução',
    'categories_desc' => 'Personaliza os grupos que usas para etiquetar.',
    'categories_empty' => 'Ainda não há categorias. Adiciona a tua primeira categoria ou escolhe o teu país para carregar as predefinidas.',

    'from_corpus' => 'do corpus',
    'rename_input_aria' => 'Novo nome para :name',
    'rename' => 'Mudar o nome',
    'rename_aria' => 'Mudar o nome de :name',
    'rename_save_aria' => 'Guardar o novo nome de :name',
    'save' => 'Guardar',
    'archive' => 'Arquivar',
    'archive_aria' => 'Arquivar :name',

    'new_category_label' => 'Nome da nova categoria',
    'new_category_placeholder' => 'Nome da nova categoria…',
    'add_category' => 'Adicionar categoria',
    'category_added' => 'Categoria adicionada.',
    'category_gone' => 'Categoria não encontrada (pode ter sido eliminada noutro separador).',

    'archived_count' => 'Arquivadas (:count)',
    'restore' => 'Restaurar',
    'restore_aria' => 'Restaurar :name',
];
