...
<div class="max-w-2xl mx-auto space-y-6" data-testid="open-banking-settings-page">
        ...
        <div class="flex items-start justify-between gap-3">
            <div class="flex-1">
                ...
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    @if ($enabled)
                        {{ Lang::get('openbanking::messages.page.toggle_connected', ['bank' => $connectedBanks]) }}
                    @else
    ...
    {{-- One card per connected bank, each holding its own consent, its own
         freshness and its own Sync now. Nothing on this page has to choose
         which of two connected banks it speaks for. --}}
    @foreach ($connectionIds as $cardConnectionId)
        @livewire(
            'openbanking.open-banking-connection-card',
            ['connectionId' => $cardConnectionId],
            key('ob-connection-'.$cardConnectionId)
        )
    @endforeach

    @if ($enabled)
        <div class="flex flex-wrap items-center justify-between gap-3">
            {{-- Adding a bank is not re-acknowledging anything: the warning is
                 answered once, and the wizard opens straight at the picker. --}}
            <x-core::secondary-button
                class="min-h-[44px]"
                wire:click="connectAnotherBank"
                data-testid="ob-connect-another-button"
            >{{ Lang::get('openbanking::messages.page.connect_another') }}</x-core::secondary-button>
    ...
    {{-- One confirm for both entry points: the on-to-off toggle click and the
         Disconnect button under the connection list. --}}
