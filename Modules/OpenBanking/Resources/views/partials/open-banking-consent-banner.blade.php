...
@if ($enabled && $consentStatus === 'expired')
    ...
    >
        <p class="text-sm font-semibold text-rose-700 dark:text-rose-300">Consent expired — reconnect</p>
        <p class="mt-1 text-sm text-rose-700 dark:text-rose-300">
            Your last successful sync was {{ $this->lastSuccessfulSyncRelative() ?? 'never' }}. Reconnect to resume automatic syncing.
        </p>
        <button
            type="button"
