...
- [x] **ING-04**: User can upload an ICS Cards PDF statement (the Mijn ICS consumer portal export format) and have its transactions imported, with original-currency + settled-EUR preserved per line where applicable
- [x] **ING-05**: User can upload a PayPal activity CSV and have its transactions imported, with the event-log rolled up by `Transaction ID` / `Reference Txn ID` so fees, holds, and currency-conversion rows enrich a single canonical transaction (rather than landing as duplicates)
