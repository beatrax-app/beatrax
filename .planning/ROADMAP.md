...
- [x] **Phase 10: Cash-Flow Forecasting + What-If Scenarios** - 30/60/90-day per-account projection with uncertainty ranges, surplus/shortfall windows, non-persisted what-if mutations with side-by-side comparison (completed 2026-05-18)
- [x] **Phase 11: Operational Hardening** - `db:backup` via `VACUUM INTO`, restore-verification, launchd polish, post-scan summaries, daily-use reliability (completed 2026-05-19)
...
**Plans**: 6 plans
  - [x] 10-01-PLAN.md — Wave 0 enablement: Modules/Forecasting module skeleton + ForecastingServiceProvider with Phase 8/9 event listener wiring + Routes/web.php + 3-step PSR-4 registration + 5 BoundaryArchTest invariants (noFacadeCallsFromForecasting / noTransactionWritesFromForecasting / crossModuleAccessGoesThroughPublic / noSynchronousForecastingInRequestLifecycle / load-bearing noScenarioMutationsJoinedToTransactionQueries) + 3 listener scaffolds + 10-scenario synthesised forecasting fixture corpus + FixtureCorpusTest
  - [x] 10-02-PLAN.md — Wave 1 schema + DTOs + state machine + Chains DTO extension: 5 migrations + 4 Eloquent models + 4 factories + 8 Public DTOs (5-subtype ScenarioMutationPayload union) + ScenarioMutationPayloadCast + ForecastRunStateMachine + Modules\Chains\Public\Dto\NextSettlementDto + CardStatementQuery::nextSettlementForUser
  - [x] 10-03-PLAN.md — Wave 2 baseline vertical slice: BalanceAnchorResolver + RangeProjector envelope tier + DailyFold quadrature + ProjectionPipeline + ProjectForecastJob ('baseline' sentinel) + ProjectForecastOnRecurringChange listener body + ForecastQuery + ScenarioQuery + /forecast Livewire skeleton + rangeArea chart partial + ForecastingProjectionContractTest + ForecastCrossUser404Test (FCT-01, FCT-02)
  - [x] 10-04-PLAN.md — Wave 3 chain routing + shortfall + dashboard consolidation: ChainAwareForecastRouter + ShortfallDetector with buffer_used_minor honest-audit + ForecastShortfallDetected event + SetAccountForecastBuffer Action + AccountBufferEditor popover + ForecastHighlightsQuery + Phase 5 "Next ICS settlement" tile REPLACED by new "Forecast highlights" Livewire tile + top-nav Forecast slot with rose-50 ↘ pill (FCT-01, FCT-05)
  - [x] 10-05-PLAN.md — Wave 4 scenarios CRUD + side-by-side + launchpads: 3 Public Events + 6 core + 3 launchpad Public Actions + ScenarioApplier in-memory transform + ProjectionPipeline scenario branch + ProjectForecastOnScenarioChange listener body + side-by-side two-panel ForecastPage with shared y-axis + Net diff tile + ScenarioEditorSidebar + Phase 9 drift "Model cancel ↗" chip + Phase 8 "/recurring/series/{id}" "Model what-if ↗" link + wire:poll.2s overlay (FCT-03, FCT-04)
  - [x] 10-06-PLAN.md — Wave 5 percentile tier + polish + FCT-03 runtime proof: Percentile R-7 + CadenceJitter + RangeProjector percentile-tier branch + ChainAwareForecastRouter viewByFunder collapse + SetAccountOpeningBalance + OpeningBalanceEditor + Confidence legend sidebar + All accounts aggregate line chart + multi-currency polish + ScenarioIsolationContractTest runtime FCT-03 proof (FCT-02, FCT-03, FCT-04)
**UI hint**: yes
...
**Plans**: 5 plans
  ...
  - [x] 11-04-PLAN.md — Wave 3 banner + failed-jobs CLI + arch locks: SystemAlertsBanner Livewire SFC + Blade view + app.blade.php slot + DurationParser + FailedJobsCommand (diederik:failed-jobs prune --older-than --dry-run) + BoundaryArchTest::noFacadeCallsFromCoreConsoleCommands + HorizonForceFlagTest (FND-05 SC #2)
  - [x] 11-05-PLAN.md — Wave 4 close-out: README ## Backups + ## Operator recovery rewrite (4 new subsections; preserves Stuck Redis lock subsection) + ReadmeOperationalDocsTest content arch test + Phase11AcceptanceTest end-to-end vertical + human-verify checkpoint (FND-05 SC #3)
