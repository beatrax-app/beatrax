...
**Plans**: 5 plans
  - [x] 08-01-PLAN.md — Wave 0 enablement: Modules/Recurring/ skeleton + ApexCharts install + four BoundaryArchTest invariants + MerchantMemoryQuery batch method + Wave 0 fixture corpus + contract test scaffold
...
**Plans**: 6 plans
  - [ ] 10-01-PLAN.md — Wave 0 enablement: Modules/Forecasting module skeleton + ForecastingServiceProvider with Phase 8/9 event listener wiring + Routes/web.php + 3-step PSR-4 registration + 5 BoundaryArchTest invariants (noFacadeCallsFromForecasting / noTransactionWritesFromForecasting / crossModuleAccessGoesThroughPublic / noSynchronousForecastingInRequestLifecycle / load-bearing noScenarioMutationsJoinedToTransactionQueries) + 3 listener scaffolds + 10-scenario synthesised forecasting fixture corpus + FixtureCorpusTest
  - [ ] 10-02-PLAN.md — Wave 1 schema + DTOs + state machine: 5 migrations (forecast_scenarios + forecast_scenario_mutations + forecast_shortfall_windows + forecast_runs + add_forecast_columns_to_accounts) + 4 Eloquent models with BelongsToUser + 4 factories + 8 Public DTOs including 5-subtype ScenarioMutationPayload Spatie LaravelData union + ScenarioMutationPayloadCast typed Eloquent cast + ForecastRunStateMachine sole-mutator with locked transition map
  - [ ] 10-03-PLAN.md — Wave 2 baseline vertical slice: BalanceAnchorResolver (ASN/ICS/PayPal anchor strategy) + RangeProjector envelope tier + DailyFold quadrature (load-bearing sqrt(2)*10 = 14 math) + ProjectionPipeline + ProjectForecastJob (ShouldBeUniqueUntilProcessing with 'baseline' sentinel) + ProjectForecastOnRecurringChange listener body + ForecastQuery + ScenarioQuery + /forecast Livewire skeleton + rangeArea chart partial + ForecastingProjectionContractTest 6-fixture dataset + ForecastCrossUser404Test (FCT-01, FCT-02)
  - [ ] 10-04-PLAN.md — Wave 3 chain routing + shortfall + dashboard consolidation: ChainAwareForecastRouter (Phase 5 ChainLinkQuery + CardStatementQuery integration; ASN deducts ICS bulk-iDEAL settlement) + ShortfallDetector with buffer_used_minor honest-audit + ForecastShortfallDetected event + SetAccountForecastBuffer Action + AccountBufferEditor popover + ForecastHighlightsQuery (badge + tile read API) + Phase 5 "Next ICS settlement" tile REPLACED by new "Forecast highlights" Livewire tile (Phase 5 NextIcsSettlementTileTest assertions PORTED) + top-nav Forecast slot with rose-50 ↘ pill (FCT-01, FCT-05)
  - [ ] 10-05-PLAN.md — Wave 4 scenarios CRUD + side-by-side + launchpads: 3 Public Events (ScenarioCreated / ScenarioMutated / ScenarioDeleted) + 6 core Public Actions (CreateScenario / RenameScenario / DeleteScenario / Add/Remove/EditScenarioMutation) + 3 launchpad Actions (CreateCancellationScenarioForAlert + CreateCancellationScenarioForSeries + CreateAmountChangeScenarioForSeries) + ScenarioApplier in-memory transform + ProjectionPipeline scenario branch + ProjectForecastOnScenarioChange listener body + side-by-side two-panel ForecastPage layout with shared y-axis + Net diff tile + ScenarioEditorSidebar with 5-option Add chooser + Phase 9 drift `Model cancel ↗` chip + Phase 8 `/recurring/series/{id}` `Model what-if ↗` link + wire:poll.2s overlay (FCT-03, FCT-04)
  - [ ] 10-06-PLAN.md — Wave 5 percentile tier + polish + FCT-03 runtime proof: Percentile R-7 helper + CadenceJitter ±3-day window + RangeProjector percentile-tier branch (variance ≥ 40% AND n ≥ 6) with envelope fallback + ChainAwareForecastRouter viewByFunder=true collapse + SetAccountOpeningBalance Action with €500 soft-warning soft-trigger + OpeningBalanceEditor /settings extension + Confidence legend sidebar (high/medium/low chip thresholds) + All accounts aggregate `line` chart (D-1027 default landing) + multi-currency edge-case polish + dedicated ScenarioIsolationContractTest (runtime end-to-end FCT-03 proof complementing the structural arch test from Plan 10-01) (FCT-02, FCT-03, FCT-04)
**UI hint**: yes
