...
| 010 | phase-17-identify-unknown-flow | Where does the "label this unknown counterparty" CTA live and what does the flow feel like? | **B** — single dedicated triage queue at `/counterparties/triage`. One unknown at a time, progress bar, confidence-based suggestion banner ("Looks like Ziggo — confidence high" with reasoning), keyboard-first (Y/N/S/→). Modal (A) and inline-editor (C) variants dropped — all "Label this" CTAs (sketch 009 card grid + sidebar Triage item) route to the queue. | counterparties, triage, identification, phase-17 |
