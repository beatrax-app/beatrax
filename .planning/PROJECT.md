...
---
*Last updated: 2026-05-19 — phase 11 complete (operational hardening; 5 plans / 4 waves; `php artisan db:backup` VACUUM INTO with chmod 0600 + meta sidecar + retention pruning + smart-skip via PRAGMA data_version + daily 03:00 schedule; `php artisan db:restore` triple safety rail with pre-restore snapshot; `php artisan diederik:doctor` extended with WAL / synchronous / backup-freshness probes; `php artisan diederik:failed-jobs prune --older-than --dry-run`; non-halting app-boot PRAGMA health check; system_alerts persistence surface + SystemAlertsBanner Livewire SFC mounted in app layout; README ## Backups + ## Operator recovery rewritten; Phase11AcceptanceTest end-to-end vertical; arch invariants noFacadeCallsFromCoreConsoleCommands + noLaravelGlobalHelpersInCoreConsoleCommands + HorizonForceFlagTest; 1644 project tests green; 15/15 code-review findings fixed across 15 commits; 7 human-verify items deferred to 11-HUMAN-UAT.md until after the desktop-app phase)*
