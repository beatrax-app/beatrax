...
|--------|-----------|-----------|-------------------|-------------|--------|
| FND-01 | App refuses non-loopback bind | Feature | `pest tests/Feature/LoopbackOnlyTest.php -x` | ✅ Plan 02 | ✅ green (Plan 02) |
| FND-02 | Single-user Fortify + Livewire login works | Feature | `pest tests/Feature/Auth/LoginFlowTest.php -x` | ✅ Plan 02 | ✅ green (Plan 02) |
| FND-03 | Every domain table has nullable `user_id` | Arch | `pest tests/Contracts/UserIdColumnArchTest.php -x` | ✅ Plan 01 | ✅ green (Plan 03) |
| FND-04 | No `REAL`/`FLOAT` on money columns | Arch | `pest tests/Contracts/NoFloatMoneyArchTest.php -x` | ✅ Plan 01 | ✅ green (Plan 03) |
| FND-06 | SQLite WAL + `synchronous=NORMAL` on connection | Unit | `pest Modules/Core/tests/Unit/SqlitePragmasTest.php -x` | ✅ Plan 02 | ✅ green (Plan 02) |
| FND-07 | Money arithmetic uses brick/money exclusively | Unit | `pest Modules/Ledger/tests/Unit/MoneyValueObjectTest.php -x` | ✅ Plan 03 | ✅ green (Plan 03) |
...
| LED-01 | Accounts have type + currency | Unit | `pest Modules/Ledger/tests/Unit/AccountModelTest.php -x` | ✅ Plan 03 | ✅ green (Plan 03) |
| LED-02 | Transaction.type enum populated | Unit | `pest Modules/Ledger/tests/Unit/TransactionTypeTest.php -x` | ✅ Plan 03 | ✅ green (Plan 03) |
| MC-01 | Both original + settled amount columns present | Arch | `pest tests/Contracts/MoneyColumnsArchTest.php -x` | ✅ Plan 01 | ✅ green (Plan 03) |
...
| CAT-05 | Triage page lists uncategorized rows | Feature | `pest Modules/Categorization/tests/Feature/TriagePageTest.php -x` | ✅ Plan 07 | ✅ green (Plan 07) |
...
| PLT-01 | Localhost-only middleware in place | Feature | (same as FND-01) `pest tests/Feature/LoopbackOnlyTest.php -x` | ✅ Plan 02 | ✅ green (Plan 02) |
| PLT-02 | DB path validation in install command | Feature | `pest Modules/Core/tests/Feature/InstallCommandTest.php::test_refuses_icloud_path -x` | ✅ Plan 02 | ✅ green (Plan 02) |
