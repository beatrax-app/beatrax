...
- A per-inbox OAuth user-authorization flow (the "Connect Gmail" button) that runs against `http://127.0.0.1:PORT/oauth/callback/{provider}` (RFC 8252 loopback IP redirect; no public webhook URL needed; both providers reject `https://*.test` subdomains, see D-114) and stores the refresh token in a chmod-600 JSON file.
...
- **D-111:** **Bring-your-own OAuth client per install, configured as "In production" with the user as sole owner.** Each diederik installation registers its own Google Cloud project + Azure App Registration. No single shipped diederik `client_id`. Google's `gmail.readonly` is a sensitive scope and Microsoft Graph's `Mail.Read` is similarly gated — verification of a single shipped client would require a paid security review, demo video, privacy-policy URL, domain verification, and weeks of back-and-forth. Bring-your-own bypasses the verification gate: each install is its own Google Cloud project, owned by the user. **The wizard mandates pushing the GCP project's OAuth consent screen to "In production" status** — this is free, requires no Google review, and is the only state that issues non-expiring refresh tokens for sensitive scopes when the project has a single internal user. Leaving the project in "Testing" status expires refresh tokens after 7 days for `gmail.readonly` ([Google OAuth 2.0 Policies](https://developers.google.com/identity/protocols/oauth2/policies)) — unacceptable for an always-on scanner. Azure App Registrations have no equivalent gate; "single tenant, accounts in this organizational directory only" with the user-owned tenant is sufficient. Trade-off: the installer adds one extra wizard step (single click "Publish App" in GCP console). Verification of a published multi-household "diederik" app is a future option if distribution ever matters.
...
- **D-114:** **First-launch per-provider modal wizard for OAuth-client registration, on a loopback IP redirect URI.** When the user clicks "Add Gmail" or "Add Microsoft 365" on `/inboxes` and no client is configured for that provider, a Flux modal opens with numbered steps + screenshots + "Open Google Cloud Console" / "Open Azure Portal" deep-link buttons + paste-back fields for `client_id`, `client_secret`, plus a copy-to-clipboard for the exact redirect URI Google/Microsoft requires: **`http://127.0.0.1:8000/oauth/callback/{provider}`** (or whichever port the user's Herd / `php artisan serve` binds — wizard surfaces the actual port from `config('app.url')` with a fallback). Both providers reject `https://*.test` subdomains as redirect URIs ([Google OAuth Native App spec](https://developers.google.com/identity/protocols/oauth2/native-app) requires loopback IP `http://127.0.0.1:PORT`; [Microsoft reply-URL docs](https://learn.microsoft.com/en-us/entra/identity-platform/reply-url) allow HTTP only on literal `localhost`, never on `.test` subdomains). The loopback IP scheme is RFC 8252-explicit and never leaves the device. The wizard's GCP step additionally requires the user to **toggle the OAuth consent screen to "In production"** (single click; D-111). Submit writes the chmod-600 JSON via `OAuthSecretsRepository`. After registration completes, the modal closes and the user is redirected directly into the provider's OAuth consent flow without an extra click. The wizard never reappears once that provider has a client configured (until the user explicitly resets via `/settings`). Wave 0 amends `PROJECT.md` to document the redirect URI scheme alongside the existing `https://diederik.test` development URL.
...
- **`Modules/EmailScan/Internal/Http/Controllers/OAuthCallbackController.php`** — handles `http://127.0.0.1:PORT/oauth/callback/{provider}` redirect-URI hits (loopback IP scheme; see D-114).
