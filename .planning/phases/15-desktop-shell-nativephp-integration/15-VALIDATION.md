...
| 15-01-T2 | 01 | 1 | PKG-04/05 | T-15-02 | cleanup_env_keys strips secrets; NativePHP facade-fake availability recorded | feature | php artisan about \| grep nativephp + test -f NATIVEPHP-FAKES.md | ❌ W0 | ⬜ |
...
| 15-06-T1 | 06 | 2 | PKG-05 | T-15-18 | theme value validated, not echoed raw; dark-companion guard wired + scoped | feature+arch | pest --filter=ThemePreference + pest --filter=dark | ❌ W0 | ⬜ |
| 15-06-T2 | 06 | 2 | PKG-05 | T-15-19 | shell/Auth/Ledger dark companions enforced | arch+build | pest --filter=dark + npm run build | ❌ W0 | ⬜ |
| 15-06-T3 | 06 | 2 | PKG-05 | — | Import/Forecasting dark companions enforced | arch+build | pest --filter=dark + npm run build | ❌ W0 | ⬜ |
...
| 15-07-T1 | 07 | 3 | PKG-05 | T-15-20 | dark guard enforces Categorization/Chains/Receipts | build+arch | npm run build + pest --filter=dark | ❌ W0 | ⬜ |
| 15-07-T2 | 07 | 3 | PKG-05 | T-15-21 | dark guard enforces EmailScan/Recurring/DriftAlerts/Desktop | build+arch | npm run build + pest --filter=dark | ❌ W0 | ⬜ |
...
- [ ] `tests/Contracts/BoundaryArchTest.php` — add the dark-companion arch guard (15-06-T1), scoped to exclude only the plan-07 modules; closed to full coverage in 15-07-T3
- [ ] `Modules/Desktop/Internal/Native/NATIVEPHP-FAKES.md` — record NativePHP v2 facade-fake availability (Window/Menu/MenuBar/Notification) in 15-01-T2 for plan 15-02's TDD tasks
...
| Native chrome present (window/menu/tray/notifications/dark-follows-OS) | PKG-05 | Visual + OS-integration behavior; also covers any 15-02 assertion `->todo()`-deferred because NativePHP v2 ships no fake for that facade | Launch built app, inspect window, app menu, system tray menu, fire a notification, toggle OS dark mode |
...
**Approval:** planner-populated 2026-05-22 (revised — checker feedback)
