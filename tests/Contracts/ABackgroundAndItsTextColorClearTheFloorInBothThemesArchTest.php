...
use Tests\Contracts\Support\ColorPairs;
use Tests\Contracts\Support\ThemeColor;
 ...
 */
function colorPairsInEveryTemplate(): array
{
    $pairs = [];

    foreach (ColorPairs::templates() as $template) {
        $pairs = array_merge(
            $pairs,
            ColorPairs::inInlineStyles($template['path'], $template['source']),
            ColorPairs::inPhpStrings($template['path'], $template['source']),
        );
    }
 ...
 */
function colorPairsInTheSaveButtonAsItShipped(): array
{
    $path = 'tests/Contracts/Fixtures/Contrast/a-save-button-drawn-in-hardcoded-white.blade';
    $source = (string) file_get_contents(base_path($path));

    return ColorPairs::measure(ColorPairs::inInlineStyles($path, $source));
}
...
it('converts an oklch value rather than reading its numbers as channels', function (): void {
    $blue = ThemeColor::resolve('var(--color-blue)', ThemeColor::DARK);
    $nearWhite = ThemeColor::resolve('oklch(99% 0 0)', ThemeColor::DARK);
    ...
    expect(round(ThemeColor::ratio($nearWhite, $blue), 2))->toBe(2.47);
    expect(round(ThemeColor::ratio([99.0, 0.0, 0.0, 1.0], $blue), 2))->toBe(5.40);
});

it('resolves a token to the value each theme gives it', function (): void {
    expect(ThemeColor::resolve('var(--color-text)', ThemeColor::LIGHT))->toBe([15.0, 23.0, 42.0, 1.0]);
    expect(ThemeColor::resolve('var(--color-text)', ThemeColor::DARK))->toBe([241.0, 245.0, 249.0, 1.0]);
    expect(ThemeColor::resolve('var(--color-text-inverse)', ThemeColor::LIGHT))->toBe([248.0, 250.0, 252.0, 1.0]);
    expect(ThemeColor::resolve('var(--color-text-inverse)', ThemeColor::DARK))->toBe([15.0, 23.0, 42.0, 1.0]);
});

it('reports the Save button that shipped white on a token that turns white at night', function (): void {
    $report = colorPairsInTheSaveButtonAsItShipped();
...
it('leaves alone a background with no text, a transparent ground and a style it cannot read', function (): void {
    ...
    expect(ColorPairs::opaqueInlineStyles($path, $source))->toHaveCount(1);
});
...
it('never lets a template declare a pair that vanishes in one theme', function (): void {
    $pairs = colorPairsInEveryTemplate();
    $report = ColorPairs::measure($pairs);
...
it('never lets a rule declare a pair that vanishes in one theme', function (): void {
    ...
    $report = ColorPairs::measure(
        ColorPairs::inStylesheet($stylesheet, (string) file_get_contents(base_path($stylesheet)))
    );
