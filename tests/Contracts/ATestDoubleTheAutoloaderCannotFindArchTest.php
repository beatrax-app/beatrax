...
const DECLARATIONS_NO_AUTOLOADER_REACHES = [
    ...
    'tools/PhpStan/Rules/Fixtures/BadFingerprintTupleFixture.php declares BadFingerprintTupleFixture' => 'FingerprintTupleRule only reads an update on the transactions table, so the fixture has to sit in a Ledger namespace to be a subject of the rule at all; were it autoloadable it would be a shipped seeder writing an amount without its digest, which is the defect it exists to reproduce. PhpStanFingerprintTupleRuleTest points PHPStan at the file by path',
    'tools/PhpStan/Rules/Fixtures/GoodFingerprintTupleFixture.php declares GoodFingerprintTupleFixture' => 'the clean half of the FingerprintTupleRule pair, which has to sit in the same namespace as the offending half or it would prove the rule silent for the wrong reason',
