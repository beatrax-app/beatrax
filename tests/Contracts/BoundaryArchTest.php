...
const BOUNDARY_ALLOW_LIST_PINS = [
    ...
    'noHorizonImportsInShippedBuildCode' => [
        'Modules/DevMode/Providers/HorizonServiceProvider.php' => [
    ...
    'noShellContractOutsideAllowList' => [
        'Modules/Desktop/Internal/Native/DesktopUrlOpener.php' => [
            'reason' => 'the desktop implementation of ExternalUrlOpener, and the only file that may name the desktop Shell for a URL; the three files this replaced were all in Modules/Community, which the phone also runs',
...
it('does not allow raw path helpers or hard-coded storage literals outside UserDataPathService (noStoragePathHardCodedOutsideUserDataPathService)', function (): void {
    ...
    $hits = [];
    $scanned = 0;
    foreach (['Modules', 'config'] as $root) {
        $abs = base_path($root);
        if (! is_dir($abs)) {
            continue;
        }
...
it('does not allow Horizon imports outside the allow-listed provider (noHorizonImportsInShippedBuildCode)', function (): void {
    ...
    $hits = [];
    $scanned = 0;
    foreach (['Modules', 'bootstrap', 'routes'] as $root) {
        $abs = base_path($root);
        if (! is_dir($abs)) {
            continue;
        }
    ...
    expect($hits)->toBe(
        [],
        "Only Modules/DevMode/Providers/HorizonServiceProvider.php may reference Laravel\\Horizon\\* symbols. Offenders:\n  ".implode("\n  ", $hits),
    );
});
...
it('restricts Native\\Desktop\\Contracts\\Shell imports to the allow-listed action and fallback (noShellContractOutsideAllowList)', function (): void {
    // The Shell contract is the desktop's outbound system-browser path, and one
    // file may import it: the DesktopUrlOpener behind ExternalUrlOpener. It used
    // to be three files in Modules/Community, which is a module the phone also
    // runs and cannot load a desktop-only type in.
...
it('pins every cross-module raw-table write to the allow-list (crossModuleRawTableWrites)', function (): void {
    ...
    $pinned = [
        ...
        // `demo:seed --reset` tears down the dataset it is about to rebuild.
        // It reads import_runs stamped source_format='demo' and deletes the
        // transactions hanging off them, which is bounded to demo data by that
        // stamp and by the demo usernames. Ledger has no seam for "delete
        // every row an import run owns" and should not grow one for a
        // developer command; the write stays here, where the bound is visible,
        // and every deleted row is announced as a tombstone afterwards.
        'Modules/DevMode/Internal/Console/DemoSeedCommand.php import_runs 1',
        'Modules/DevMode/Internal/Console/DemoSeedCommand.php transactions 1',
...
it('does not allow a cross-module Internal import outside the pinned production and test sets (pinnedCrossModuleInternalImports)', function (): void {
    ...
    $pinnedTestCrossings = [
        ...
        'Modules/Counterparties/tests/Feature/ANameThisDeviceCouldNotOpenIsNotReassertedTest.php -> Modules\\Sync\\Internal\\Crypto\\GdkKeyringService',
        ...
        'Modules/Ledger/tests/Feature/ALegNoteTheEditorNeverShowedSurvivesASaveTest.php -> Modules\\Sync\\Internal\\Crypto\\GdkKeyringService',
        ...
        'Modules/Ledger/tests/Feature/AnAppendDoesNotSwallowANoteItCouldNotReadTest.php -> Modules\\Sync\\Internal\\Crypto\\GdkKeyringService',
        ...
        'Modules/Mobile/tests/Feature/AnImportDoesNotCompleteOverAHistoryItCannotProjectTest.php -> Modules\\Sync\\Internal\\Crypto\\GdkKeyringService',
        'Modules/Mobile/tests/Feature/AnImportDoesNotCompleteOverAHistoryItCannotProjectTest.php -> Modules\\Sync\\Internal\\Identity\\DeviceIdentityService',
        'Modules/Mobile/tests/Feature/AnImportDoesNotCompleteOverAHistoryItCannotProjectTest.php -> Modules\\Sync\\Internal\\Transport\\Relay\\RelayConfig',
        ...
        'Modules/Mobile/tests/Feature/PairingManualCodeArmTest.php -> Modules\\Sync\\Internal\\Transport\\Discovery\\LocalNetworkGate',
        'Modules/Mobile/tests/Feature/PairingManualCodeArmTest.php -> Modules\\Sync\\Internal\\Transport\\Discovery\\NativeBridge',
        ...
        'Modules/Tax/tests/Feature/ASecondTagOnTheSameRowIsAChangeNotACreateTest.php -> Modules\\Sync\\Internal\\OpLog\\OpLogWriter',
        ...
        'tests/Feature/ARefusedKeyStoreWriteNeverReachesTheSessionTest.php -> Modules\\Auth\\Internal\\Lock\\LockStateManager',
        'tests/Feature/ARefusedKeyStoreWriteNeverReachesTheSessionTest.php -> Modules\\Desktop\\Internal\\Native\\DesktopKeyCustodian',
        'tests/Feature/ARefusedKeyStoreWriteNeverReachesTheSessionTest.php -> Modules\\Desktop\\Internal\\Native\\SafeStorageBackendProbe',
        ...
        // private.
        // The statement rebasers and the setup command were App\ classes until
        // app/ went; these three tests named them all along and the crossing
        // only became visible when they landed in a module. Each test IS the
        // proof of that class's behaviour, so the subject cannot be swapped for
        // a seam, and DevMode should not widen its Public surface to hold a
        // developer fixture tool.
        'tests/Unit/AFixtureIsDatedTheWayTheParserWillReadItTest.php -> Modules\\DevMode\\Internal\\Fixtures\\MonthShift',
        'tests/Unit/AFixtureIsDatedTheWayTheParserWillReadItTest.php -> Modules\\DevMode\\Internal\\Fixtures\\Mt940Rebaser',
        'tests/Unit/AFixtureIsDatedTheWayTheParserWillReadItTest.php -> Modules\\Ingestion\\Internal\\Adapters\\Banking\\Mt940Tag61Parser',
        'tests/Unit/ARebasedStatementKeepsTheShapeItWasReadForTest.php -> Modules\\DevMode\\Internal\\Fixtures\\Camt053Rebaser',
        'tests/Unit/ARebasedStatementKeepsTheShapeItWasReadForTest.php -> Modules\\DevMode\\Internal\\Fixtures\\MonthShift',
        'tests/Unit/ARebasedStatementKeepsTheShapeItWasReadForTest.php -> Modules\\DevMode\\Internal\\Fixtures\\Mt940Rebaser',
        'tests/Unit/ARebasedStatementKeepsTheShapeItWasReadForTest.php -> Modules\\DevMode\\Internal\\Fixtures\\PresetCsvRebaser',
        'tests/Unit/SetupCommandEnvEncodingTest.php -> Modules\\DevMode\\Internal\\Console\\SetupCommand',
    ];
    ...
    foreach (['Modules', 'tests'] as $dir) {
        ...
        foreach ($iterator as $file) {
            ...
            if (str_starts_with($relative, 'tools/PhpStan/Rules/Fixtures/')) {
                continue;
            }
